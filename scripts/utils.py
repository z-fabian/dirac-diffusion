...
class EMA(Callback):
    ...
    def compare_tensors(self, model):
        tot_norm = 0
        for param in model.parameters():
            tot_norm += torch.linalg.norm(param.data)
        return tot_norm.detach().cpu().data
            
    def on_validation_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        # rank_zero_info(f"Start of validation before swapping {self.compare_tensors(pl_module)}")
        if self._should_validate_ema_weights(trainer):
            self.swap_model_weights(trainer)
        # rank_zero_info(f"Start of validation after swapping {self.compare_tensors(pl_module)}")

    def on_validation_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        # rank_zero_info(f"End of validation before swapping {self.compare_tensors(pl_module)}")
        if self._should_validate_ema_weights(trainer):
            self.swap_model_weights(trainer)
        # rank_zero_info(f"End of validation after swapping {self.compare_tensors(pl_module)}")
